...
export function Medications() {
  ...
  const [refillMed, setRefillMed] = useState<typeof medications[number] | null>(null);
  ...
  function openRefill(med: typeof medications[number]) {
    setRefillMed(med);
    setRefillReason('');
  }

  function submitRefill() {
    if (!refillMed || !currentPatient || patientDoctors.length === 0) return;
    const primaryDoctor = patientDoctors[0];
    if (!primaryDoctor) return;
    addMedicationRequest({
      patientId: currentPatientId,
      patientName: currentPatient.name,
      medication: `${refillMed.name} ${refillMed.dosage} (refill)`,
      reason: refillReason,
  ...
  return (
    <div className="space-y-6">
      ...
      <Dialog open={!!refillMed} onOpenChange={(open) => !open && setRefillMed(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Request Refill — {refillMed?.name} {refillMed?.dosage}</DialogTitle>
            <DialogDescription>
              Your request will be sent to your primary care physician for approval.
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
            ...
            <Textarea
              placeholder="e.g., Running low, upcoming travel..."
              value={refillReason}
          ...
          <DialogFooter>
            ...
            <Button onClick={submitRefill} disabled={!refillReason.trim()}>
              Submit Request
            </Button>
          </DialogFooter>
        </DialogContent>
