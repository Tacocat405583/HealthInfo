...
import { Activity, Shield, MessageSquare, Zap, CheckCircle, ArrowRight, Users, Clock, Wallet } from 'lucide-react';
import { useWallet } from '../hooks/useWallet';
import { useApp } from '../app/context/AppContext';

export default function LandingPage() {
  const navigate = useNavigate();
  const { isConnected, connect, isConnecting, connectError, wrongNetwork, shortAddress, switchNetwork } = useWallet();
  const { setUserRole } = useApp();

  async function enterAs(role: 'patient' | 'doctor') {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <nav className="sticky top-0 z-50 bg-card/90 backdrop-blur-sm border-b border-border px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          {wrongNetwork ? (
            <button onClick={() => void switchNetwork()} className="px-4 py-2 bg-yellow-500 text-white rounded-lg text-sm">
              Switch Network
            </button>
          ) : isConnected ? (
            <span className="px-3 py-1.5 bg-green-100 text-green-800 rounded-lg text-sm font-mono flex items-center gap-2">
            ...
            <button
              ...
              className="px-4 py-2 bg-primary text-white rounded-lg hover:opacity-90 transition-opacity text-sm disabled:opacity-50"
            >
              {isConnecting ? 'Connecting…' : 'Connect Wallet'}
            </button>
          )}
          <button
            onClick={() => void enterAs('patient')}
            className="px-4 py-2 border border-border text-foreground rounded-lg hover:bg-accent transition-colors text-sm"
          >
            Patient Sign In
          </button>
          <button
            onClick={() => void enterAs('doctor')}
            className="px-4 py-2 bg-primary text-white rounded-lg hover:opacity-90 transition-opacity text-sm"
          >
            Doctor Sign In
          </button>
        </div>
        {connectError && <p className="absolute top-16 right-6 text-xs text-red-500">{connectError}</p>}
      </nav>
      ...
      <section className="flex-1 flex flex-col items-center justify-center text-center px-6 py-24 bg-gradient-to-br from-background via-muted/30 to-accent/30">
        ...
        <div className="flex items-center gap-4 flex-wrap justify-center">
          <button
            onClick={() => void enterAs('patient')}
            disabled={isConnecting}
            className="flex items-center gap-2 px-6 py-3 bg-primary text-white rounded-lg hover:opacity-90 transition-opacity font-medium disabled:opacity-50"
          >
            {isConnecting ? 'Connecting…' : 'Access Patient Portal'}
            <ArrowRight className="w-4 h-4" />
          </button>
          <button
            onClick={() => void enterAs('doctor')}
            disabled={isConnecting}
            className="flex items-center gap-2 px-6 py-3 border border-border text-foreground rounded-lg hover:bg-accent transition-colors font-medium disabled:opacity-50"
          >
            Provider Dashboard
            <ArrowRight className="w-4 h-4" />
          </button>
