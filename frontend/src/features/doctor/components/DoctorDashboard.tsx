...
export function DoctorDashboard({ onTabChange }: { onTabChange?: (tab: string) => void }) {
  const { currentDoctorId, doctors } = useApp();
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-foreground mb-2">Good morning, {currentDoctor?.name ?? 'Doctor'}</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-card border border-border rounded-xl p-6">
            ...
            {pendingLabs.map((lab, i) => (
              <div key={i} className="flex items-center gap-3 p-3 bg-muted rounded-lg">
                ...
                <button
                  ...
                  className="text-xs px-3 py-1.5 bg-primary text-white rounded-lg hover:opacity-90 transition-opacity flex-shrink-0"
                >
                  Review
                </button>
              </div>
          ...
          <button
            onClick={() => onTabChange?.('labs')}
            className="w-full mt-4 py-2 text-primary hover:bg-primary/5 rounded-lg transition-colors text-sm"
          >
            View all lab orders
          </button>
        </div>
