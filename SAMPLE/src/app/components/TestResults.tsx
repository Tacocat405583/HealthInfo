...
type FilterTab = 'All Results' | 'Lab Work' | 'Imaging' | 'Vital Signs';

export function TestResults() {
  const [activeFilter, setActiveFilter] = useState<FilterTab>('All Results');

  const filtered = results.filter(
    r => activeFilter === 'All Results' || r.category === activeFilter
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-3 flex-wrap">
        {(['All Results', 'Lab Work', 'Imaging', 'Vital Signs'] as FilterTab[]).map((filter) => (
          <button
            key={filter}
            ...
            <div key={result.id} className="bg-card border border-border rounded-xl overflow-hidden hover:shadow-md transition-shadow">
              <div className="p-6 border-b border-border">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <button className="flex items-center gap-2 px-3 py-2 border border-border text-foreground rounded-lg hover:bg-accent transition-colors">
              ...
              <div className="p-6 bg-muted/30">
                ...
                {/* AI Panel — patient-facing lab explanation */}
                <RecordAIPanel
                  key={result.id}
                  id={result.id}
                  label={result.name}
